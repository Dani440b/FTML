...
DETAILED_OUT = "results/winobias/type1_pro_detailed.csv"
SUMMARY_OUT  = "results/winobias/type1_pro_summary.csv"


def run_pro_only(model_name, precision):
    model, tokenizer = load_model(model_name, precision)
    sentences = load_winobias_type1_single("data/winobias", "pro")
    ...
    print(f"[WinoBias T1 PRO] {precision}: mean={mean_lp:.3f}, std={std_lp:.3f}")


if __name__ == "__main__":
    MODEL = "FacebookAI/roberta-base"
    for p in ["fp32", "fp16", "8bit", "4bit", "2bit"]:
        run_pro_only(MODEL, p)
